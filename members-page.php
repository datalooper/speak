  ...
  <a class="docTitle" data-doctitle="<?php echo wp_title( ' | ', 'false', 'right' ); bloginfo( 'name' ); ?>"></a>
	...
	<ul class="small-block-grid-1 large-block-grid-3 xlarge-block-grid-4">
  	...
  	foreach(get_users() as $user){
        $img = get_cupp_meta($user->id, 'full');
        if($img) {
            ?>
            ...
            <li class="user <?php echo $user->id; ?>">
                ...
                <div class="userInfo">
                    <h2 class="userName left"><?php echo get_the_author_meta('first_name', $user->id) . " " . get_the_author_meta('last_name', $user->id); ?>
                        <span class="userNickname"><?php echo get_the_author_meta('nickname', $user->id); ?></span></h2>
