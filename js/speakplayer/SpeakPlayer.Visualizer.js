...
SpeakPlayer.Visualizer = {
    ...
    initVisualizer: function () {
        ...
        Sketch.create({
            setup: function () {
                ...
                for (i = _i = 0, _ref = SpeakPlayer.Visualizer.NUM_PARTICLES - 1; _i <= _ref; i = _i += 1) {
                    x = random(this.width);
                    y = random(this.height * 2);
                    particle = Particle.create(x, y);
                    particle.energy = random(particle.band / 256);
                    SpeakPlayer.Visualizer.particles.push(particle);
                }
            },
            draw: function () {
                var particle, _i, _len, _ref, _results;
                this.globalCompositeOperation = 'lighter';
                _ref = SpeakPlayer.Visualizer.particles;
                _results = [];
    ...
    initAnalyzer: function (audioEl) {
        if (this.AudioAnalyser.enabled) {
            try {
                var analyser = this.AudioAnalyser(audioEl, this.NUM_BANDS, this.SMOOTHING);
                analyser.onUpdate = (function (_this) {
                    return function (bands) {
                        var _j, _len, _ref1, _results;
                        _ref1 = SpeakPlayer.Visualizer.particles;
                        _results = [];
...
Particle = {
    ...
    reset : function () {
        ...
        this.spin = random(this.SPIN.MAX, this.SPIN.MAX);
        this.band = floor(random(this.NUM_BANDS));
        if (random() < 0.5) {
            this.spin = -this.spin;
        }
        ...
        this.decayScale = 0.0;
        this.decayAlpha = 0.0;
        this.rotation = random(this.TWO_PI);
